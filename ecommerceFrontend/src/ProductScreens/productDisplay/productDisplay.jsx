...
import { ToggleFavourites } from "../WishListScreen/WishListFunction.jsx";
...
export default function ProductDisplay(props){
    ...
    }, [context["cart"],product]) // "product" is also added to this list detect url changes which may eventually also change the product and its properties


    // this is for updating the isFavourite variable
    // so that we know if the product is present in the wishlist/favourites or not
    useEffect(() => {
        // first the product must be found! => the product must be valid product
        // The user must be signed in => only that user can use cart
        if(notFound===false && context["user"] !== null){
            let exists_in_favourites = false;
            for(let fav of [...context["favourites"]]){
                if(fav.product_id === product.product_id){
                    exists_in_favourites = true
                }
            }
            // we now know that if the item is present in wishlist
            if(exists_in_favourites){setIsFavourite(true)}
            else if(!exists_in_favourites){setIsFavourite(false)}            
        }
    }, [context["favourites"],product])
                    ...
                    <div className="d-flex mt-4 justify-content-between mr-3">
                        ...
                        <div className="text-center flex-grow-1 pt-1">
                            <div className="border p-3">
                                ...
                                className="btn btn-outline-theme btn-prodview mb-4 mt-4">
                                ...
                                <div className="btn-group mb-4 mt-4">
                                    ...
                                    disabled={quantityInCart===product.maxQuantityForAUser}>
                                        <IoAddSharp  size="1.5em" />
                                    </button>
                                </div>
                            }
                            {isFavourite === true ? 
                                <button onClick={()=>checkLoggedIn(context,()=>ToggleFavourites(context,product))}
                                className="btn btn-outline-danger mt-4 mb-4 btn-prodview">
                                    <CiHeart  size="1.8em" />
