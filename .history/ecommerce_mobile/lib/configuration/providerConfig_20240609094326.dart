...
class MyProvider extends ChangeNotifier {
    ...
    List wishlist= []; // this var stores our wishlist-items
    ...
    dynamic goToBranch; // will be used for navigating to a branch => this function will be present in mainWrapper.dart
    ...
    setCartPropertyForFavourites(){
        for(var item in wishlist){
          item["_id"] = item["product_id"]; // as quantityInCart function uses _id field to check
          int  quantity = quantityInCart(item, cart); // will return 0 if the product is absent from the cart
          item["quantityInCart"] = quantity; // then that quantity will be set for this product
        }
    }
