...
class BottomMoveButton extends StatelessWidget {
            ...
            actions: [
              ...
              OutlinedButton.icon(
                onPressed: () async {
                  ...
                  Provider.of<MyProvider>(context,listen: false).goToBranch(1); // moving to cartscreen
                  Navigator.of(context).pop(); // then remove this dialogScreen
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("moving complete", style: TextStyle(color: Colors.white)), backgroundColor: Colors.green, showCloseIcon: true)
                  );
                },
  ...
  Widget build(BuildContext context) {
    return BottomAppBar(
      elevation: 30,
      color: Color.fromARGB(196, 221, 232, 238),
