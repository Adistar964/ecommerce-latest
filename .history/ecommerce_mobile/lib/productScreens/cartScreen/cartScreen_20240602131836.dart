...
class CartScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body : (context.watch<MyProvider>().cart.length == 0 ? // or u could use Provider.of method too
              EmptyCartBody() // show this emptyCart widget if cart is empty
              : CartScreenBody()  // otherwise show this widget
              ),
      bottomNavigationBar: context.watch<MyProvider>().cart.length != 0 ?  // only show this bottomAppBar if there is something in the cart
      BottomProceedButton()
      : Text("") // otherwise we will not show this BottomProceed button appbar
    );
  }
}
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        IconButton.outlined(
                          onPressed: ()=>addQuantity(context, widget.product, (){Provider.of<MyProvider>(context,listen:false).setHomeData(Provider.of<MyProvider>(context,listen:false).homeData);}),
                          color: Colors.blue,
                          style: IconButton.styleFrom(
                            fixedSize: Size(10, 10),
                            side: BorderSide(color: Colors.blue)
...
class BottomProceedButton extends StatefulWidget {
  const BottomProceedButton({
    super.key,
  });

  @override
  State<BottomProceedButton> createState() => _BottomProceedButtonState();
}

class _BottomProceedButtonState extends State<BottomProceedButton> {
