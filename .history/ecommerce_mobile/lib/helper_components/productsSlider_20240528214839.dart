...
class _ProductCardState extends State<ProductCard> {
  ...
  void initState() {
    ...
    if(context.mounted && context.read<MyProvider>().cart != null){ // first let everything get loaded
  ...
  subtractQuantity() async {
    try{
      ...
      dynamic quantity = widget.product["quantity"]; // current quantity of our product
      dynamic newCart = [ ...context.read<MyProvider>().cart ]; // our current cart (we will make changes to it)
      if(quantity == 1){ // is quantity is 1, and if user clicked "-" button, we will remove the product from the cart itself
      ...
      // then we send this newly modified cart to our backend
      Map body = {
  ...
  addQuantity() async {
    try{
      dynamic quantity = widget.product["quantity"]; // current quantity of our product
      dynamic maxUserQuantity = widget.product["maxUserQuantity"]; // max quantity a user can add in the cart
      dynamic newCart = [ ...context.read<MyProvider>().cart ]; // our current cart (we will make changes to it)
      if(quantity == maxUserQuantity){ // if quantity reached its maxLimit, and if user clicked "+" button, we will show an alert
