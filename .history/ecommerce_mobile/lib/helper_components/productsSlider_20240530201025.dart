...
class _ProductCardState extends State<ProductCard> {
  ...
  void initState() {
    ...
    if(context.mounted){ // first let everything get loaded (double check)
        //callAssignCartQuantity() will check if the product is in the cart and assigns it a proper quantity if it exists, otherwise assigns product a quantity of 0
        callAssignCartQuantity(); // this function simply calls assignCartQuantity function with appropriate parameters
        // callAssignCartQuantity is basically qssignCartQuantity function, but shortened code
  ...
  callAssignCartQuantity(){
    assignProductCartQuantity(
      ...
      absentInCart_callback:(item)=> setState(() => productCartQuantity = item["quantity"]), // if it is absent, update the productCartQuantity variable created above to zero
    );
  }
