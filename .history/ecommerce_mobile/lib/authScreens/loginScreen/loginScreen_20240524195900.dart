...
import 'package:ecommerce_mobile/configuration/constants.dart'; // getting constants like currency
...
class LoginScreen extends StatelessWidget {
                    ...
                    Container(
                      ...
                      child: ClipRRect( // a shorthand class to give borderradius for rounded borders
                        ...
                        child: MaterialButton( // a very customizable button
                          onPressed: ()=>validateAndLogin(context), // this func will handle login things
                          color: Colors.black,
  ...
  validateAndLogin(context){
    // this firsts validates whatever user wrote in the login-form and then login the user
    var email = emailFieldController.text;
    var password = passwordFieldController.text;
    
    // if user wrote a proper email address in the email-field 
    // and also if password is greater than 8 charachters
    // only then we proceed to firebase-login 

    if( !EmailValidator.validate(email) ){ // if user did not input a proper email
    ...
    }else if(password.length < 8){ // if user inputted password less than 8 charachters
        ...
        doLogin(context); // then login the user if everything is validated
    }
  }
      ...
      // then do login stuff
        ...
        FirebaseAuth.instance.signInWithEmailAndPassword(
          ...
          password: passwordFieldController.text, // value of Password TextField
              ...
              alert(context, "Login Error", "Username or password is incorrect");
          });
    }
