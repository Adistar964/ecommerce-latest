  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        children: [
          InkWell(
            borderRadius: BorderRadius.circular(50), // so it splash effects only the space that the the child-circle is currently taking, not the whole thing
            onTap: (){},
            child: Ink( // wrapped with Ink instead of Container
              height: 50,
              width: 50,
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(50), // completely a circle! any large radius would make it circular
                color: color,
              ),
              child: Icon(icon, color: Colors.white, size: 35), // "icon" var is what we created in the top
            ),
          ),
