  ...
  void initState() {
    ...
    if(context.mounted){ // first let the app be mounted/set-up
        ...
        if(context.read<MyProvider>().dataLoaded == false){
          ...
          // whenever we fetch a lot of data, or do expensive computation, our program freezes as async stops eveyrthing until that expensive/heavy task is done
          // but isloate.run runs it parallely so fluttter doesnt stop
          ()async{await Isolate.run(() => getData());setState(() {
            loading=false;
          });};
        }
    }
