...
export default function WishList(props){
    ...
    useEffect(() => {
        ...
        // first we will make sure everything is fetched from backend beforehand
        if(context["loading"] === false && context["favourites"]){
            ...
            if(context["user"]!==null && context["favourites"].length!==0){
                setEmptyWishList(false) // its not empty
            }else{
    ...
    const showSelected = (e,idx)=>{
        ...
        // Also any changes on selection, should make the selectAllFavs input unchecked
        document.getElementById("selectAllFavs").checked = false; 
    }
    ...
    const addSelectedToCart = async () =>  {
        ...
        if(selected_items.length !== 0){
            ...
            toast.error("Please make selections before adding to the cart!")
        }
    }

    // below will be for selecting all items
    const addAllItemsToSelection = e => {
        const checked = e.target.checked
        if(checked){ // user ticked/checked it
            // we will check all the items (add them to selection)
            context["favourites"].map((prod,idx) => {
                document.getElementById("wishlist-check-"+String(idx)).checked = true
            })
        }else if(!checked){ // if user unchecked it
            // we will uncheck all the items (remove them from selection)
            context["favourites"].map((prod,idx) => {
                document.getElementById("wishlist-check-"+String(idx)).checked = false
            })
        }
    }
    ...
    if(loading){
        ...
        if(emptyWishList){ // if wish-list not there(empty)
            return(
                <HeaderAndFooter>
                <div className="text-center p-4 container border border-primary shadow-lg">
                    <BsFillBagHeartFill size="10em" className="d-block text-info m-auto mb-4" />
                    <h3>
                        <i>
        ...
        }else if(emptyWishList===false && context["favourites"]!==undefined){ // if wish-list available(not empty)
            return(
                <HeaderAndFooter>
                    <div className="p-3 container border border-secondary">
                        ...
                        <ul style={{listStyle:"none"}}>
                            {/* the below will help us in selecting all items for adding them to cart */}
                            <div className="w-100 border border-dark p-3 bg-secondary-subtle">
                                <input type="checkbox" id="selectAllFavs" 
                                onChange={addAllItemsToSelection}
                                style={{transform:"scale(1.8)"}} />
                                <p className="d-inline">&nbsp;&nbsp;&nbsp; Select All</p>
                            </div>
                            {/* The below is lists all products in wishlist/favourites */}
                            ...
                            className="w-100 border d-flex p-3 align-items-center cart-item-hover">
                                ...
                                <div className="p-3">
                                    ...
                                    <select onChange={e=>{
                                        // we will change the price displayed if the quantity changes
                                        // basically the actual price gets multiplied by the quantity user has selected 
                                        const quantity_selected = Number(e.target.value);
                                        const actual_price = item.price-item.discountPrice
                                        document.getElementById("wish-price"+idx).textContent = actual_price * quantity_selected
                                    }} 
