...
export default function WishList(props){
    ...
    // this is for later putting selected items to cart
    const [wantedInCart, setWantedInCart] = useState([]);
    ...
    // for checkboxes
    // if ticked, we want to add that item in our WantedInCart
    useEffect(() => {
        console.log(checks)
        if(checks.length !== 0){ // first let it load completely
            checks.map((checkboxes,idx) => {
                if(checkboxes.checked){
                    let item = document.getElementById("wishlist-"+idx)
                    item.style.backgroundColor = "red!important"
                }
            })
        }

    }, [checks]) // anytime the user checks/unchecks an item
                    ...
                    <div className="p-3 text-center container border border-secondary">
                        ...
                        <ul style={{listStyle:"none"}}>
                            ...
                            className="w-100 border d-flex p-3 align-items-center cart-item-hover">
                                ...
                                // below is format for using useState as ref
                                ref={newref => // whenever its value changes,
                                    setChecks(oldChecks => { 
                                        oldChecks[idx] = newref // we will change it in useState
                                        return oldChecks;
                                })} // will be in the useRef and we can access useRef to know if its check or not
                                type="checkbox" />
