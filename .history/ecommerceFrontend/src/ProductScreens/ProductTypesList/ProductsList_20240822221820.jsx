        ...
        const getProducts = async () => {
            ...
            try{
                ...
                if(data["searchResults"].length !== 0){
                    ...
                    // but we do it manually instead (not backend) as this time our requirements are different
                    // first: getting maxPrice
                    let maximPrice = 0;
                    data["searchResults"].map(product => {
                        const prodPrice = product["price"] - product["discountPrice"] // the actual price of the product
                        if (prodPrice > maximPrice){maximPrice = prodPrice}
                    })
                    // second: getting list of all brands in the results
                    let brands = []
                    data["searchResults"].map(product => {
                    ...
                    setSelectedMaxPrice(maximPrice);
                    setBrandsList(brands); // list of all the brands the search-result contains from which the user chooses
                    setSelectedBrands(brands); // initially, all brands are selected
