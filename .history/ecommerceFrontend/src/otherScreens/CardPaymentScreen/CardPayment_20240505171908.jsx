...
function CardForm(){
    ...
    // setting stripe objects
    const elements = useElements() // for accessing that stripe <CardElement />
    const stripe = useStripe() // for using stripe's functions to make payments

    const submit = async e => {
        e.preventDefault();

        // Now gettting our sensistive card info from CardElement Component
    ...
    return(
        <form onSubmit={submit} className="container w-50">
            <div className="form-group">
                <label htmlFor="emailCreditCard" className="mb-1">Email:</label>
                <input value={context["user"]["email"]} 
                type="email" className="form-control" id="emailCreditCard" />
            </div>
            <div className="form-group mt-3">
                <label htmlFor="cardElement" className="mb-1">Card Info:</label>
