...
function SearchPage() {
    ...
    useEffect(()=>{
        const getProducts = async () => {
            setLoading(true)
            try{
                const response = await fetch(backend_link+"/getSearchResults/"+search_text+"/"+sortBy+"/0/")
                const data = await response.json()
                setNumber_of_pages(data["numberOfPages"])
                setSrchResults(data["searchResults"])

                // then we will also set the values for filter-options' variables
    ...
    const sortResults = e => {
        ...
        setSortBy(sort_by)
        fetch(`${backend_link}/getSearchResults/${search_text}/${sort_by}/0/`)
        .then(res => res.json())
        .then(data => {
            setSrchResults(data["searchResults"]);
        })
    ...
    // Whenever a filter is set, like max-price, this will filter the searchResults and set it to filteredResults
    function filterResultsFunc() {
        ...
        srchResults.map(product => {
            ...
            }else if(product["brand"] in filters["brands"]){
                filteredProducts.push(product);
            }else if(product["category"] in filter["categories"]){
                filteredProducts.push(product);
            }
        })
    }
    ...
    const gotoPage = idx => {
        fetch(`${backend_link}/getSearchResults/${search_text}/${sortBy}/${idx}/`)
        .then(res => res.json())
        .then(data => {
            setSrchResults(data["searchResults"]);
