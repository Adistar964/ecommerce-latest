...
export default function CartScreen(){
    ...
    const changeQuantity = async (item,operation_type) => {
        let cart_items = [...context["cart"]] // we will update our cart by adding quantity
        for (let product of cart_items){
            if(product["product_id"] === item["product_id"]){
                // basically a shorthand if-else statement
                // if add, then add quantity
                // else if subtract, then remove quantity
                // else remove_item_from_cart, then just remove it from cart
                operation_type==="add" ? product["quantity"] ++ : 
                (operation_type=="subtract" ? product["quantity"]-- : 
                cart_items.splice(cart_items.indexOf(product),1));
            }
        }
        const body = {
            ...
            <div className="p-3 text-center container border border-secondary">
                ...
                <ul style={{listStyle:"none"}}>
                    ...
                    <li className="w-100 border d-flex p-3 align-items-center">
                        ...
                        <button onClick={()=>changeQuantity(item,"subtract")} 
                            ...
                            <button onClick={()=>changeQuantity(item,"add")} 
                            className="btn btn-outline-theme">
                                +
                            </button>
                        </div>
                        ...
                        className="d-flex justify-content-end">
                            <button onClick={()=>changeQuantity(item,"remove item from cart")} 
