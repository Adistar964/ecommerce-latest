...
function DeliveryTimingSelection(props){
    ...
    const [deliveryDate, setDeliveryDate] = useState(new Date()) // this will be the date varaible of the calendar

    useState(() => {
        let now = new Date()
        setSlots_as_per_date(now)
    }, []) // runs initially with today's delivery-date 
    ...
    async function setSlots_as_per_date(selected_date) { // this will update available/unavailable slots as the chosen date changes
        setDeliveryDate(selected_date) // so that the date changes in the calendar
        const date = selected_date.toISOString() // converting into string so that it can be sent to backend
        // new Date().toISOString
        // now fetching slots from backend:
        alert(date)
        var body = {
            date:date,  // getting slots for this date!
            // selected_date_is_today: date === new Date().toDateString() // true, if user selected today's date
        }
        const requestParams = {
            method:"POST",
        ...
        setAllSlots(slots_data) // setting new data for slots

    }

    // this returns the maximum date the user can select in the calendar (1 month for now)
    function maxDateFunction(){
        let maxDate = new Date();
        let current_month = maxDate.getMonth()
        if(current_month == 11){ // if it is december, then the user can select dates upto max january month
            maxDate.setMonth(0)
        }else{
            maxDate.setMonth(current_month+1) // usually, user can select upto one month max for delivery
        }
        return maxDate
    }
                ...
                value={deliveryDate} onChange={
                    function (selectedDate){
                        ...
                        if(day == 5){ // 5 stands for "Friday"
                            ...
                            // we will uncheck the selected radio button as we want to clear the selection made for the previous date
                            document.getElementsByName("chckbx-slot").forEach(radio => radio.checked=false)
                            props.setSelectedSlot(null) // back to default => no slot selected
                            
                            setSlots_as_per_date(selectedDate);                             
                        }
                    }
                } />
            </div>
            ...
            {allSlots.map(slot => 
                <div className="m-5 mt-0 mb-1 d-flex justify-content-around align-items-center border shadow-lg p-2 bg-white">
                    <div className="flex-grow-1 d-flex">
                        ...
                        <div className="mt-4">
                            <p>
                                {slot["startTime"]}:00 to {slot["endTime"]}:00
                            </p>
                        </div>
                    </div>
                    {slot["available"] ? 
