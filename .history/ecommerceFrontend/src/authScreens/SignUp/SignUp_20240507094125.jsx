...
export default function SignUp(){
    ...
    function createAccount(){
        ...
        if (password === retypePassword){
            ...
            .then(async credentials => {
                ...
                addUserDocAfterRegistering(uid,email)
            })
            .catch(error=>{
    ...
    const createAccountWithGoogle = () => {
        ...
        signInWithPopup(auth,GoogleProvider).then(async credentials=>{
            ...
            console.log(uid)
            addUserDocAfterRegistering(uid,email)
        }).catch(err=>{setLoading(false);toast.error("user login cancelled!");})
    }

    const addUserDocAfterRegistering = (uid,email) => {
        // we will also create the user's documnt in mongodb which will store cart,favourites,...
        const body = {
            uid:uid,
            cart_items:[],
            user_favourites:[],
        ...
        fetch(`${backend_link}/addUserDoc/`,requestParams) 
