...
def getSearchResults(request,search_text,sort_by,page_number):
    try:
        ...
        if sort_by=="relevance": # normal one(no sorting)
            searchResultsFromMongo = products_collection.find( {"$text":{"$search":search_text}} ).skip(page_number*items_in_1_page).limit(items_in_1_page)
            # .skip and .limit for pagination!
            # ** operator is basically a spread operator for dictionaries
        elif sort_by=="priceHighest": # price highest to lowest
            searchResultsFromMongo = products_collection.find( {"$text":{"$search":search_text}} ).sort({"price":-1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
        elif sort_by=="priceLowest": # price lowest to highest
            searchResultsFromMongo = products_collection.find( {"$text":{"$search":search_text}} ).sort({"price":1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
        elif sort_by=="discount": # sort by discount (highest discountPrice comes first)
            searchResultsFromMongo = products_collection.find( {"$text":{"$search":search_text}} ).sort({"discountPrice":-1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
        elif sort_by=="alphabetical": # sort alphabetically (A-Z)
            searchResultsFromMongo = products_collection.find( {"$text":{"$search":search_text}} ).sort({"title":1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
