...
# for searching:
@api_view(["POST"])
def getSearchResults(request,search_text,sort_by,page_number): # search_text,sort_by,page_number are args passed in the url 
    try:
        ...
        #     "title":"text"
        # })
        filters = request.data["filters"] # the filters passed in from frontend (max-price,min-price,etc..)
        if(search_text=="everything"): # everything is like a code-word. If used as search_text, it will understand that we dont have any specific search_text and that we need all the results based on the filter
            search_query = filters
        else:
            search_query = { "$text":{"$search":search_text}, **filters } # search query for the monogoDB
            # above: Basically we r using text-search feature from mongoDB and then also passing any extra filters if received from the frontend
        
        searchResultsFromMongo = [] # The search results will be added to this list and then it will be returned to the frontend
        items_in_1_page = 18 # can be changed
        if sort_by=="relevance": # normal one(no sorting)
            searchResultsFromMongo = products_collection.find( search_query ).skip(page_number*items_in_1_page).limit(items_in_1_page)
            # .skip and .limit for pagination!
            # ** operator is basically a spread operator for dictionaries
        elif sort_by=="priceHighest": # price highest to lowest
            searchResultsFromMongo = products_collection.find( search_query ).sort({"price":-1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
        elif sort_by=="priceLowest": # price lowest to highest
            searchResultsFromMongo = products_collection.find( search_query ).sort({"price":1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
        elif sort_by=="discount": # sort by discount (highest discountPrice comes first)
            searchResultsFromMongo = products_collection.find( search_query ).sort({"discountPrice":-1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
        elif sort_by=="alphabetical": # sort alphabetically (A-Z)
            searchResultsFromMongo = products_collection.find( search_query ).sort({"title":1}).skip(page_number*items_in_1_page).limit(items_in_1_page)
        ...
        number_of_items = products_collection.count_documents( filter=search_query )
...
# The below function returns the values for filter-options' variables so that the filters are shown with proper values when user chooses from them
# filter values return the maxPrice
# and returns categories and brands of all the searchResults so User can select some/all of them to filter out
@api_view(["POST"])
def getFilterValues(request):
    searchQuery = request.data["search_text"]
    filters = request.data["filters"] # the filters passed in from frontend (max-price,min-price,etc..)
    if(searchQuery=="everything"): # everything is like a code-word. If used as searchQuery, it will understand that we dont have any specific searchQuery and that we need all the results based on the filter
        searchQuery = filters
    else:
        searchQuery = { "$text":{"$search":searchQuery}, **filters } # search query for the monogoDB
        # above: Basically we r using text-search feature from mongoDB and then also passing any extra filters if received from the frontend

    maxPrice = products_collection.find( searchQuery, {"_id":0, "price":1,"discountPrice":1} ).sort({"price":-1}).limit(1) [0]
    # Above statement: We sort search-results in descending order of price and then we select the first row, i.e, the one which contains highest value of price
    # also we did {"_id":0, "price":1,"discountPrice":1} to make sure only these fields come out, i.e, price-field
    # [0] is to make sure that we get that only item in the list
    maxPrice = maxPrice["price"] # and then from that object we access the only field in it, i.e, the price

    brands = products_collection.distinct("brand", searchQuery) # distinct() method which will return the array of distinct values for the brand field from the search results.
    # .distinct needed because: we dont want smth like [..,"nike","nike",..] in brands list, we want all the brands' name from the srch-results but they shouldnt be repeated
    
    categories = products_collection.distinct("category", searchQuery) # distinct() method which will return the array of distinct values for the category field from the search results.
    # .distinct needed because: we dont want smth like [..,"fresh food","fresh food",..] in categories' list, we want all the categories from the srch-results but they shouldnt be repeated

    # no returning all the data for the filter options
    return Response({"maxPrice":maxPrice,"brands":brands,"categories":categories})
...
#######  Below are some miscellaneous functions which help us getting the list of all brands, categories, etc...

# this will return a list of all categories of products like mobiles,fresh-food,etc...
@api_view(["GET"])
def fetchAllProductCategories(request):
    allCategories = products_collection.distinct("category") # getting a distinct list of all categories
    return Response(list(allCategories)) 

# this will return a list of product-types corresponding to a category
# for example: sea-food is product type that comes under "fresh food" category
@api_view(["GET"])
def fetchAllProductTypes(request, category):
    # category = request.data["category"] # we will fetch all product-types corresponding to this category
    allTypes = products_collection.distinct("type", {"category":category}) # we will fetch all distinct product-types corresponding to this category

    return Response(list(allTypes))

# this will return a list of brands corresponding to a category
# for example: samsung is brand that comes under "electronics" category
@api_view(["POST"])
def fetchAllProductBrands(request):
    category = request.data["category"] # we will fetch all brands corresponding to this category
    allBrands = products_collection.distinct("brand", {"category":category}) # we will fetch all distinct brands corresponding to this category

    return Response(list(allBrands))
