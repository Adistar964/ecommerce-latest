...
def getFilterValues(request, search_text):
    filterOptions = {"brands":[],"categories":[],"maxPrice":0} # we will fill this and return it to the frontend
    
    maxPrice = products_collection.find( {}, {"_id":0, "price":1,"discountPrice":1} ).sort({"price":-1}).limit(1) [0]
    # Above statement: We sort everything in descending order of price and then we select the first row, i.e, the one which contains highest value of price
    ...
    maxPrice = maxPrice["price"] - maxPrice["discountPrice"] # for the actual price 

    filterQuery = {"$text":{"$search":search_text}}
    brands = products_collection.distinct("brand", filterQuery) # distinct() method which will return the array of distinct values for the brand field from the search results.
    ...
    categories = products_collection.distinct("category", filterQuery) # distinct() method which will return the array of distinct values for the category field from the search results.
